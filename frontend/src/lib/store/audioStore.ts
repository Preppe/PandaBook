...
interface AudioActions {
  ...
  skip: (seconds: number, direction: "forward" | "backward") => void; // Added action to skip forward or backward
}
...
const useAudioStore = create<AudioState & AudioActions>()(
  persist(
    (set, get) => ({
      ...
      skip: (seconds, direction) => {
        const { audioElement, duration } = get();
        if (audioElement) {
          let newTime = audioElement.currentTime;
          if (direction === "forward") {
            newTime = Math.min(audioElement.currentTime + seconds, duration);
          } else if (direction === "backward") {
            newTime = Math.max(audioElement.currentTime - seconds, 0);
          }
